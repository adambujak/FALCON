 ...
 *                           bit set indicates on, bit clear indicates off.
 *
 *   ACCEL_ACCURACY_SET  0x4000 - accel accuracy when changes, HEADER2_SET also needs to be set in data output control
 * regsiter 1 GYRO_ACCURACY_SET   0x2000 - gyro accuracy when changes, HEADER2_SET also needs to be set in data output
 * control regsiter 1 CPASS_ACCURACY_SET  0x1000 - compass accuracy when changes, HEADER2_SET also needs to be set in
 * data output control regsiter 1 BATCH_MODE_EN       0x0100 - enable batching
 */
 ...
 *   data output control register 1, data output control register 2, data interrupt control register, motion event
 * control regsiter, data ready status register
 */
 ...
 *   [1] = ACCEL_CAL_A_VAR
 *   [2] = ACCEL_CAL_DIV - divider from hardware accel engine rate such that acce cal runs at
 * accel_engine_rate/(divider+1)
 */
inv_error_t dmp_set_accel_cal_params_20648(int *accel_cal)
{
  ...
  result
          = inv_dmpdriver_write_mems(ACCEL_ALPHA_VAR, 4, inv_dmpdriver_int32_to_big8(accel_cal[ACCEL_CAL_ALPHA_VAR], big8));
...
inv_error_t dmp_set_compass_cal_params_20648(int *compass_cal)
{
  ...
  result = inv_dmpdriver_write_mems(CPASS_TIME_BUFFER, 2,
                                    inv_dmpdriver_int16_to_big8(compass_cal[CPASS_CAL_TIME_BUFFER], big8));
  result
    += inv_dmpdriver_write_mems(CPASS_RADIUS_3D_THRESH_ANOMALY, 4,
                                inv_dmpdriver_int32_to_big8(compass_cal[CPASS_CAL_RADIUS_3D_THRESH_ANOMALY], big8));
 ...
 *           DMP takes raw accel data and left shifts by 16 bits, so 1g=2^14 (<<16) becomes 1g=2^30, to make 1g=2^25,
 * >>5 bits. In Q-30 math, >> 5 equals multiply by 2^25 = 33554432.
 *
 *           For 8g parts, 8g = 2^15 -> 1g = 2^12.
 *           DMP takes raw accel data and left shifts by 16 bits, so 1g=2^12 (<<16) becomes 1g=2^28, to make 1g=2^25,
 * >>3bits. In Q-30 math, >> 3 equals multiply by 2^27 = 134217728.
 */
