...
inv_error_t inv_initialize_lower_driver(enum MEMS_SERIAL_INTERFACE type, const unsigned char *dmp_image_sram)
{
...
#if (MEMS_CHIP == HW_ICM20648)
  // Check board version
  if (data & 0x8) return INV_ERROR_INVALID_CONFIGURATION;
#endif
  ...
  if (data != 0x04) base_state.lp_en_support = 0;
#endif
  if (base_state.lp_en_support == 1) inv_set_chip_power_state(CHIP_LP_ENABLE, 1);
...
inv_error_t inv_set_slave_pressure_id(void)
{
  ...
  if (result) desactivate_pressure();
...

  {
    ...
    if (base_state.timebase_correction_pll & 0x80) {
      ResultLL = (MagicConstant * (long long)(1UL << gyro_level) * (1 + div)
                  / (1270 - (base_state.timebase_correction_pll & 0x7F)) / MagicConstantScale);
    }
    else {
      ResultLL = (MagicConstant * (long long)(1UL << gyro_level) * (1 + div)
