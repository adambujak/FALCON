...
static inv_error_t inv_recover_setting()
{
  ...
  // Reset DMP
  result |= inv_write_single_mems_reg_core(REG_USER_CTRL, (saved_regs.user_ctrl & (~BIT_FIFO_EN)) | BIT_DMP_RST);
 ...
 */
static int inv_check_accelgyro_self_test(enum INV_SENSORS sensorType, uint8_t *selfTestValuesReadFromReg,
                                         int *meanNormalTestValues, int *meanSelfTestValues)
{
  int ret_val;
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (!lIsStOtpReadZero) {
      // Compare the current Self-Test response (GXST, GYST, GZST, AXST, AYST and AZST) to the factory Self-Test values
      // (ST_OTP) and report Self-Test is passing if all the following criteria are fulfilled: (GXST / GXST_OTP)  > 0.5
      if (lDiffNormalStValues[i] < LOWER_BOUND_CHECK(l_st_otp_read[i])) ret_val = 1;
      if (sensorType != INV_SENSOR_GYRO)
        // (AXST / AXST_OTP)  < 1.5
        if (lDiffNormalStValues[i] > UPPER_BOUND_CHECK(l_st_otp_read[i])) ret_val = 1;
    }
    else
      ret_val = 1;
  }
