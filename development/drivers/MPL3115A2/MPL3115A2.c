...
fln_i2c_handle_t *MPL3115A2_i2c_handle; 
void MPL3115A2_i2c_connect(fln_i2c_handle_t *handle)
{
    MPL3115A2_i2c_handle = handle;
}
...
{
  unsigned char data;
  bsp_i2c_read(MPL3115A2_i2c_handle, MPL3115A2_ADDRESS, reg, 1U, &data);                                      //Read register current value
  return data;
}

void MPL3115A2_ReadByteArray(uint8_t reg, uint8_t *buffer, uint8_t length)
{    
  bsp_i2c_read(MPL3115A2_i2c_handle, MPL3115A2_ADDRESS, reg, length, buffer); //Read register current value
}

void MPL3115A2_WriteByte(uint8_t reg, uint8_t value)
{
  bsp_i2c_write(MPL3115A2_i2c_handle, MPL3115A2_ADDRESS, reg, 1U, &value);                     //Write value to register
}

void MPL3115A2_WriteByteArray(uint8_t reg, uint8_t* buffer, uint8_t length)
{
  bsp_i2c_write(MPL3115A2_i2c_handle, MPL3115A2_ADDRESS, reg, length, buffer); //Write value to register
}
